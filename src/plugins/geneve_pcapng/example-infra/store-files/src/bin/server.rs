...
async fn save_file(mut body: Body, path: PathBuf, max_size: usize, buffer_size: usize) -> Result<usize, UploadError> {
            ...
            match save_file(
                ...
                Ok(size) => {
                    let response = Response::builder()
                        ...
                        .body(Body::from(format!("File uploaded successfully. Size: {} bytes", size)))
                        .unwrap();
                    Ok(response)
                }
                Err(e) => {
                    ...
                    let (status, message) = match e {
                        UploadError::SizeLimitExceeded => (
                            ...
                            format!("File size exceeds maximum allowed size of {} bytes", config.max_file_size)
                        ),
                        UploadError::Io(err) => (
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("Maximum file size: {} GB", config.max_file_size as f64 / 1_073_741_824.0);
