...
static http_buffer_vft_t buf_vfts[HTTP_BUFFER_PTR + 2];
...
static svm_fifo_seg_t *
buf_streaming_get_segs (http_buffer_t *hb, u32 max_len, u32 *n_segs)
{
  http_buffer_streaming_t *bs = (http_buffer_streaming_t *) &hb->data;
...
static u8
buf_streaming_is_drained (http_buffer_t *hb)
{
  http_buffer_streaming_t *bs = (http_buffer_streaming_t *) &hb->data;

  /* For streaming with unknown length, we're never truly "drained"
   * until the connection is closed */
  if (bs->total_len == ~0ULL)
    return 0;

  return (bs->sent >= bs->total_len);
}
...
const static http_buffer_vft_t buf_streaming_vft = {
  ...
  .is_drained = buf_streaming_is_drained,
};
...
u8
http_buffer_is_streaming (http_buffer_t *hb)
{
  return (hb->vft == &buf_vfts[HTTP_BUFFER_STREAMING]);
}
