...
typedef struct
{
    ...
    /* offset inside the preconnect buffer for sending */
    uword preconnect_offset;
    u8 headers_sent;
...
http_pcapng_chunk_write (void *context, const void *chunk, size_t chunk_size)
{
    ...
    if (chunk_size < 8192 && vec_len (ctx->preconnect_buffer) < 8192)
    {
	void *chunk_target = 0;
	vec_add2 (ctx->preconnect_buffer, chunk_target, chunk_size);
	memcpy (chunk_target, chunk, chunk_size);
	// clib_warning("enqueue small chunk");
        return 0;
    }
	...
	if (max_enq == 0) //< vec_len (ctx->preconnect_buffer))
	  {
	...
	u32 enqueue_len = vec_len (ctx->preconnect_buffer) - ctx->preconnect_offset;
	if (enqueue_len > max_enq) {
	   enqueue_len = max_enq;
	}

	int rv = svm_fifo_enqueue (ctx->session->tx_fifo,
				   enqueue_len,
				   ctx->preconnect_buffer + ctx->preconnect_offset);
	if (rv < 0)
	  {
	    return -1;
	  }

	ctx->preconnect_offset += rv;

	ctx->total_bytes_sent += rv;
	ctx->chunks_sent++;
        if (ctx->preconnect_offset == vec_len(ctx->preconnect_buffer)) {
	   vec_reset_length(ctx->preconnect_buffer);
	   ctx->preconnect_offset = 0;
	   clib_warning("Reset buffer");
	}
    }
